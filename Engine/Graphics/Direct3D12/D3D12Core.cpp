...
namespace primal::graphics::d3d12::core {
	...
	// TODO : Remove when you are done showing how to create a root signature the tedious way
	...
	namespace {
		...
		public:
			...
			explicit d3d12_command(ID3D12Device8* const device, D3D12_COMMAND_LIST_TYPE type)
			{
				...
				desc.Type = type;
				device->CreateCommandQueue(&desc, IID_PPV_ARGS(&_cmd_queue));
		...
		private:
			struct command_frame
			{
				...
				void wait(HANDLE fence_event, ID3D12Fence1* fence)
				{
					...
					// then we know the GPU has not finished executing the command_lists
		...
		IDXGIAdapter4* determine_main_adapter()
		{
			...
			for (u32 i{ 0 };
				dxgi_fctory->EnumAdapterByGpuPreference(i, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_PPV_ARGS(&adapter)) != DXGI_ERROR_NOT_FOUND;
				++i)
			{
	...
	bool initialize()
	{
		...
		HRESULT hr{ S_OK };
		DXCall(hr = CreateDXGIFactory2(dxgi_factory_flags, IID_PPV_ARGS(&dxgi_fctory)));
	...
	void shutdown()
	{
		...
		// NOTE: We dont Call process_deferred_releases at the end because
		//	some resources (such as sway chains) can't be released before
	...
	// NOTE: This function demostrates how to create a root signature as an example
	...
	void create_a_root_signature()
	{
		...
		{// param 2 : Descriptor Table (unbound / Bindless)
			auto& param = params[2];
		...
		//only one resoure heap and one sampler heap can be set at any time
		...
		cmd_list->SetGraphicsRoot32BitConstant(0, dt_unit, 0);
	...
	void create_a_pipeline_state_object()
	{
		...
		desc.pPipelineStateSubobjectStream = &strcat;
	...
	{
		struct {
			d3d12_pipeline_state_subobject_root_signature root_sig{ _root_signature };
			d3d12_pipeline_state_subobject_vs vs{ _vs };
		} stream;

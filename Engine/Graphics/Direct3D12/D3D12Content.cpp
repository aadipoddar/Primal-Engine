...
namespace primal::graphics::d3d12::content {
	...
	namespace submesh {
		...
		primal::id::id_type add(const u8*& data)
		{
			...
			// NOTE: Element size may be 0, for position-only
			...
			const u32 alignment{ D3D12_STANDARD_MAXIMUM_ELEMENT_ALIGNMENT_BYTE_MULTIPLE };
			...
			if (element_size)
			{
				view.element_buffer_view.BufferLocation = resource->GetGPUVirtualAddress();
